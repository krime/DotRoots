...
syn keyword javaTodo contained TODO FIXME XXX NOTE
...
syn region  javaCommentCharacter start="/\*"  end="\*/" contains=@javaCommentSpecial,javaTodo,@Spell
...
syn region javaFuncDef start="^\z(\s*\)\(\(public\|protected\|private\|static\|abstract\|final\|native\|synchronized\)[ \n]\+\)*\(void\|boolean\|char\|byte\|short\|int\|long\|float\|double\|\([a-zA-Z][A-Za-z0-9_]*\)\)[ \n]\+[a-z][A-Za-z0-9_]*[ \n]*(\_[^)]*)[ \n]*\(\<throws\>[ \n]\+[A-Z]\w\+\([ \n]*,[ \n]*[A-Z]\w\+\)*[ \n]*\)\?{" end="^\z1}$" keepend transparent fold

"" Uncomment this if you want to fold all {}
syn region javaFuncDef start="^\z(\s*\){" end="^\z1}" keepend transparent fold

"syn match javaFunction contained +[A-Za-z_][A-Za-z0-9_]+
"syn region javaFunctionDef start=+\z(\s*\)+ end=+\z1}+ nextgroup=javaScopeDecl,javaType,javaStorage,javaFunction keepend transparent fold skipwhite skipnl
...
"  JavaHiLink javaFunction                  Function
  ...
  JavaHiLink javaUserLabelRefjavaUserLabel Label
  ...
  JavaHiLink javaMethodDecl                MethodClass
  "JavaHiLink javaClassDecl                 javaStorageClass
  JavaHiLink javaClassDecl                 Class
  "JavaHiLink javaScopeDecl                 javaStorageClass
  JavaHiLink javaScopeDecl                 ScopeClass
